...
def _resolve_model() -> str:
    ...
    try:
        ...
        from ak_engine.integrations.hermes import resolve_model

        return resolve_model("", get_preferred_silent_default_model(), {"provider":"auto"})
    except Exception:
        return "z-ai/glm-5.2"
...
def _run_prompt_submit(
    ...
    def run():
        ...
        try:
            ...
            try:
                from ak_engine.integrations.hermes import resolve_model
                routed_model = resolve_model(
                    run_message,
                    getattr(agent, "model", ""),
                    {"provider": getattr(agent, "provider", "")},
                )
                if routed_model and routed_model != getattr(agent, "model", ""):
                    print(f"[AK Engine] Switching model: {agent.model} -> {routed_model}")
                    agent.model = routed_model
            except Exception as e:
                print(f"[AK Engine] Router error: {e}")
